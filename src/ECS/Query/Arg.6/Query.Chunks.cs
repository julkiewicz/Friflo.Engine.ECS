...
/// </summary>
public readonly struct Chunks<T1, T2, T3, T4, T5, T6>
    where T1 : struct
    where T2 : struct
    where T3 : struct
    where T4 : struct
    ...
    where T6 : struct
{
    ...
    public readonly     Chunk<T6>       Chunk6;     //  16
    public readonly     ChunkEntities   Entities;   //  32

    public override     string          ToString() => Entities.GetChunksString();

    internal Chunks(Chunk<T1> chunk1, Chunk<T2> chunk2, Chunk<T3> chunk3, Chunk<T4> chunk4, Chunk<T5> chunk5, Chunk<T6> chunk6, in ChunkEntities entities) {
        Chunk1     = chunk1;
        Chunk2     = chunk2;
        Chunk3     = chunk3;
        Chunk4     = chunk4;
    ...
    internal Chunks(in Chunks<T1, T2, T3, T4, T5, T6> chunks, int start, int length, int taskIndex) {
        ...
        Chunk6      = new Chunk<T6>    (chunks.Chunk6,   start, length);
    ...
    public void Deconstruct(out Chunk<T1> chunk1, out Chunk<T2> chunk2, out Chunk<T3> chunk3, out Chunk<T4> chunk4, out Chunk<T5> chunk5, out Chunk<T6> chunk6, out ChunkEntities entities) {
        chunk1      = Chunk1;
        chunk2      = Chunk2;
        chunk3      = Chunk3;
        chunk4      = Chunk4;
...
/// See <a href="https://friflo.gitbook.io/friflo.engine.ecs/documentation/query-optimization#enumerate-query-chunks">Example.</a>
/// </summary>
public readonly struct QueryChunks<T1, T2, T3, T4, T5, T6>  : IEnumerable <Chunks<T1, T2, T3, T4, T5, T6>>
    where T1 : struct
    where T2 : struct
    where T3 : struct
    where T4 : struct
    ...
    where T6 : struct
{
    private readonly    ArchetypeQuery<T1, T2, T3, T4, T5, T6>  query;
    ...
    internal QueryChunks(ArchetypeQuery<T1, T2, T3, T4, T5, T6> query) {
        this.query = query;
    }
    
    // --- IEnumerable<>
    [ExcludeFromCodeCoverage]
    IEnumerator<Chunks<T1, T2, T3, T4, T5, T6>>
    IEnumerable<Chunks<T1, T2, T3, T4, T5, T6>>.GetEnumerator() => new ChunkEnumerator<T1, T2, T3, T4, T5, T6> (query);
    
    // --- IEnumerable
    [ExcludeFromCodeCoverage]
    IEnumerator     IEnumerable.GetEnumerator() => new ChunkEnumerator<T1, T2, T3, T4, T5, T6> (query);
    
    public ChunkEnumerator<T1, T2, T3, T4, T5, T6> GetEnumerator() => new (query);
}

public struct ChunkEnumerator<T1, T2, T3, T4, T5, T6> : IEnumerator<Chunks<T1, T2, T3, T4, T5, T6>>
    where T1 : struct
    where T2 : struct
    where T3 : struct
    where T4 : struct
    ...
    where T6 : struct
{
    ...
    private readonly    int                     structIndex6;   //  4
    //
    private readonly    EntityStoreBase         store;          //  8
    private readonly    Archetypes              archetypes;     // 16
    //
    private             int                     archetypePos;   //  4
    private             Chunks<T1,T2,T3,T4,T5,T6>  chunks;      // 128 ???
    
    
    internal  ChunkEnumerator(ArchetypeQuery<T1, T2, T3, T4, T5, T6> query)
    {
        ...
        structIndex6    = query.signatureIndexes.T6;
    ...
    public readonly Chunks<T1, T2, T3, T4, T5, T6> Current   => chunks;
    ...
    SetChunks:
        ...
        var chunks6     = (StructHeap<T6>)heapMap[structIndex6];
        ...
        var chunk6      = new Chunk<T6>(chunks6.components, count, start);
        var entities    = new ChunkEntities(archetype,      count, start);
        chunks          = new Chunks<T1, T2, T3, T4, T5, T6>(chunk1, chunk2, chunk3, chunk4, chunk5, chunk6, entities);
        return true;
    SingleEntity:
        if (pos >= types.last) {
            return false;
