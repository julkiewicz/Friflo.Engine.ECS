...
/// </summary>
public sealed class QueryJob<T1, T2, T3, T4, T5, T6> : QueryJob
    where T1 : struct
    where T2 : struct
    where T3 : struct
    where T4 : struct
    ...
    where T6 : struct
{
    internal            QueryChunks<T1, T2, T3, T4, T5, T6>     Chunks      => new (query);     // only for debugger
    internal            QueryEntities                       Entities    => query.Entities;  // only for debugger
    public  override    string                              ToString()  => query.GetQueryJobString();

    [Browse(Never)]
    private readonly    ArchetypeQuery<T1, T2, T3, T4, T5, T6>                                              query;      //  8
    private readonly    Action<Chunk<T1>, Chunk<T2>, Chunk<T3>, Chunk<T4>, Chunk<T5>, Chunk<T6>, ChunkEntities>    action;     //  8
    [Browse(Never)]
    private             QueryJobTask[]                                                                  jobTasks;   //  8


    private class QueryJobTask : JobTask {
        internal    Action<Chunk<T1>, Chunk<T2>, Chunk<T3>, Chunk<T4>, Chunk<T5>, Chunk<T6>, ChunkEntities>    action;
        internal    Chunks<T1, T2, T3, T4, T5, T6>                                                      chunks;
        
        internal  override void ExecuteTask()  => action(chunks.Chunk1, chunks.Chunk2, chunks.Chunk3, chunks.Chunk4, chunks.Chunk5, chunks.Chunk6, chunks.Entities);
    }
    
    internal QueryJob(
        ArchetypeQuery<T1, T2, T3, T4, T5, T6>                                              query,
        Action<Chunk<T1>, Chunk<T2>, Chunk<T3>, Chunk<T4>, Chunk<T5>, Chunk<T6>, ChunkEntities>    action)
    {
        this.query  = query;
        this.action = action;
    ...
    {
        foreach (Chunks<T1, T2, T3, T4, T5, T6> chunk in query.Chunks) {
            action(chunk.Chunk1, chunk.Chunk2, chunk.Chunk3, chunk.Chunk4, chunk.Chunk5, chunk.Chunk6, chunk.Entities);
        }
    }
    ...
    public override void RunParallel()
    {
        ...
        foreach (Chunks<T1, T2, T3, T4, T5, T6> chunks in query.Chunks)
        {
            var chunkLength = chunks.Length;
            if (ExecuteSequential(taskCount, chunkLength)) {
                action(chunks.Chunk1, chunks.Chunk2, chunks.Chunk3, chunks.Chunk4, chunks.Chunk5, chunks.Chunk6, chunks.Entities);
                continue;
            }
            var tasks = jobTasks;
            ...
            for (int taskIndex = 0; taskIndex < taskCount; taskIndex++)
            {
                ...
                if (length > 0) {
                    tasks[taskIndex].chunks = new Chunks<T1, T2, T3, T4, T5, T6>(chunks, start, length, taskIndex);
                    start += sectionSize;
                    continue;
                }
                for (; taskIndex < taskCount; taskIndex++) {
                    tasks[taskIndex].chunks = new Chunks<T1, T2, T3, T4, T5, T6>(chunks.Entities, taskIndex);
                }
                break;
            }
    ...
    private static int GetMultiple()
    {
        ...
        int lcm56   =   LeastComponentMultiple(lcm5,    lcm6);
        return          LeastComponentMultiple(lcm1234, lcm56);
    }
}
